...
const s = {
  ...
  createTransaction: conn.prepare('INSERT INTO transactions (user_id, family_id, type, category, amount, note, asset_name, quantity, unit_price, fee, tx_type, current_price, bank, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'),
  ...
  updateTransaction: conn.prepare('UPDATE transactions SET category = ?, amount = ?, note = ?, asset_name = ?, quantity = ?, unit_price = ?, fee = ?, tx_type = ?, current_price = ?, bank = ?, date = ? WHERE id = ? AND family_id = ?'),
...
module.exports = {
  ...
  createTransaction: (userId, familyId, type, category, amount, note, assetName, quantity, unitPrice, fee, txType, currentPrice, date, bank) => { const r = s.createTransaction.run(userId, familyId, type, category, amount, note, assetName||'', quantity||0, unitPrice||0, fee||0, txType||'buy', currentPrice||0, bank||'', date); return { id: r.lastInsertRowid }; },
  ...
  updateTransaction: (category, amount, note, assetName, quantity, unitPrice, fee, txType, currentPrice, date, id, familyId, bank) => s.updateTransaction.run(category, amount, note, assetName||'', quantity||0, unitPrice||0, fee||0, txType||'buy', currentPrice||0, bank||'', date, id, familyId),
